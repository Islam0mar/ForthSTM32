...
@       ARM register usage
col .req 	r9 		@ absolute address of docol
...
// Store the chain of links.
	...
	.macro defword name, namelen, flags=F_IMMED, label
	.text
	.align	2
	.globl name_\label
name_\label :
	...
	.type	\label, %function
\label :
	bx col 		// enter docolon
	// list of word pointers follow
	.endm

	.macro defcode name, namelen, flags=F_IMMED, label
	.text
	.align	2
	.globl name_\label
name_\label :
...
@@@ push "b docol" on the stack for new words
	defcode "LITDOCOL",8,,LITDOCOL
	ldr r1, = cold_start
    ldr r1, [r1]
	...
	bne 2f						@ compile to ram

	@@ FLASH
	add fpc, #12				@ skip HIDDEN
...
*/
	...
	.macro defvar name, namelen, flags=F_IMMED, label, initial=0
	defcode \name,\namelen,\flags,\label
	pushTOS
	ldr tos, = var_\name
	NEXT
...
*/
	...
	defvar "DP",2,,DP
	defvar "LATEST",6,,LATEST,name_QUIT @ latest defined word RAM
...
@ in the data segment.
	...
	defcode "CREATE",6,,CREATE
		...
		ldr r1, =FLASH_CR
		ldr r1, [r1]
		tst r1, #FLASH_CR_LOCK
		beq 3f						@ compile to flash
@@@ RAM
...
1:
        ...
        strb r6,[r3], 	#1 @ and skip the word
...
4:  	ldr r7, [r1]
		...
		orr tos, 0xA0			@ hidden and immediate FLAG
		ldrb r6,[r0],	#1 @ read and store a character
		lsl r6, 8
		orr tos,r6
	
        strh tos,[r3], 	#2 @ store the length of the word
		ldr r1, =FLASH_SR
4:  	ldr r7, [r1]
    	tst r7, #FLASH_SR_BSY
    	bne 4b
		...
		bne 1f					@ odd length
	@@ even length
		ldrh r6,[r0],	#1 @ read and store a character
		orr r6, 0xFF00
        strh r6,[r3], 	#2 @ and skip the word
		ldr r1, =FLASH_SR
4:  	ldr r5, [r1]
    	tst r5, #FLASH_SR_BSY
    	bne 4b
...
1:
    	...
    	ldrh r6,[r0],	#2 @ read and store a character
        strh r6,[r3], 	#2 @ and skip the word
		ldr r1, =FLASH_SR
4:  	ldr r5, [r1]
    	tst r5, #FLASH_SR_BSY
...
2:		str     r0, [r2], #4    @ write value and increment address
3:		str     r2, [r1]        @ update DP
        bx      lr
...
@ : ( -- ) Define a new forth word
	...
	.4byte LITDOCOL	// Append DOCOL  (the codeword).
...
@ IMMEDIATE ( -- ) sets IMMEDIATE flag of last defined word
	...
	ldr r1, [r1]
	tst r1, #FLASH_CR_LOCK
	beq 1f						@ compile to flash

	//add r1, r1, #4          @ points to the flag byte
	//mov r2, #0              @
	ldrb r2, [r1, #4]!           @ load the flag into r2
                                @
...
1:	
	ldrh r2, [r1, #4]!           @ load the flag into r2
                                @
	eor r2, r2, #F_IMMED    @ r2 = r2 xor F_IMMED
	strh r2, [r1]           @ update the flag
	ldr r3, =FLASH_SR
1:  ldr r2, [r3]
    tst r2, #FLASH_SR_BSY
    bne 1b
...
@ HIDE ( -- ) hide a word // only for RAM as there is no need in FLASH
	defword "HIDE",4,,HIDE
...
1:  ldr r3, [r2]
	...
	ldr r1, =var_FLASH
	ldr r2, =var_RAM
	ldr r3, [r0]				@ var_RAM = var_DP
	str r3, [r2]				@ var_DP = var_FLASH 
	ldr r3, [r1]
	str r3, [r0]
...
@@@ update latest variable
	...
	ldr r0, [r3]
	str r0, [r2]
	str r1,	[r3]				@ swap(latest,f_latest)
...
// search latest link in RAM
	ldr r4, =var_F_LATEST
    ldr r3, [r4]                    @ get the last defined word address in ram 
...
@ Lock Flash after finishing this
	...
	ldr r1, =var_FLASH
	ldr r2, =var_RAM
	ldr r3, [r0]				@ var_FLASH = var_DP
	str r3, [r1]				@ var_DP = var_RAM 
	ldr r3, [r2]
	str r3, [r0]
