...
@ 1+ ( a | a+1 ) increments the top element
	...
	@@ ( x(low) x(high) a b -- x+a*b(low) x+a*b(high) )
	defcode "D+*",3,,SMLAL
	pop {r0, r1, r2}
	smlal r2, r1, r0, tos
	mov tos, r2
...
@ ( a b -- r ) where a = q * b + r
	defcode "UMOD",4,,UMOD
	pop {r0}
	UDIV r1, r0, tos				@ quotient
    mls tos, r1, tos, r0 @ push remainder
    NEXT

	@ ( a b -- r ) where a = q * b + r
	defcode "SMOD",4,,SMOD
	pop {r0}
	SDIV r1, r0, tos				@ quotient
    mls tos, r1, tos, r0 @ push remainder
    NEXT
...
@ = ( a b | p ) where p is 1 when a and b are equal (0 otherwise)
	...
	defcode "ORN",3,,ORN	// bitwise OR NOT
	pop {r0}
    orn tos, r0, tos
    NEXT

	defcode "CLZ",3,,CLZ	// count leading zeros
    clz tos, tos
    NEXT

	defcode "BIC",3,,BIC	// bit clear
	pop {r0}
    bic tos, r0, tos
    NEXT

	defcode "ARSHIFT",7,,ARSHIFT	// ( x1 u -- x2 ) Arithmetric right-shift of u bit-places
	pop {r0}
    asr tos, r0, tos
    NEXT

	defcode "RSHIFT",6,,RSHIFT	// ( x1 u -- x2 ) Arithmetric right-shift of u bit-places
	pop {r0}
    lsr tos, r0, tos
    NEXT

	defcode "LSHIFT",6,,LSHIFT	// ( x1 u -- x2 ) Arithmetric right-shift of u bit-places
	pop {r0}
    lsl tos, r0, tos
    NEXT
...
_NUMBER:
	    ...
	    @ if only one input skip(ox.. or ob..)
		cmp r1, #1
		beq skipNum
...
@ REG! ( -- ) 
	defcode "REG!",4,F_COMPO,REG_STORE
	push {r4-r6}
	NEXT

@ REG@ ( -- ) 
	defcode "REG@",4,F_COMPO,REG_FETCH
	pop {r4-r6}
	NEXT
